...
#----------------------------------------------------#
    # mean_restored = cv2.cvtColor(img1.astype(np.float32), cv2.COLOR_RGB2GRAY).mean()
    # mean_target   = cv2.cvtColor(img2.astype(np.float32), cv2.COLOR_RGB2GRAY).mean()
    # img1 = np.clip(img1 * (mean_target / (mean_restored + 1e-6)), 0, 255)
